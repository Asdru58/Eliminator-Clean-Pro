...
pub fn scan_directories(
    ...
    for path_str in paths {
        ...
        for entry_res in WalkDir::new(path_str).into_iter() {
            // Check for cancellation periodically
            if total_files % 1000 == 0 && cancelled.load(Ordering::Relaxed) {
                return Err("Cancelled".into());
            }
            ...
            let entry = match entry_res {
                Ok(e) => e,
                Err(e) => {
                    // If error is at depth 0, it means we can't access the chosen folder at all.
                    if e.depth() == 0 {
                        return Err(format!(
                            "Permiso denegado o error accediendo a: {}",
                            e.path()
                                .unwrap_or(std::path::Path::new("carpeta desconocida"))
                                .display()
                        ));
                    }
                    // For subfiles/folders, just skip them
                    continue;
                }
            };
    ...
    let mut potential_duplicates: Vec<Vec<String>> =
    ...
    let _ = window.emit(
        "scan-progress",
        ProgressEvent {
            phase: "Phase 1/2: Partial Hash".into(),
            current: 0,
            total: total_groups,
        },
    );

    let partial_results: Vec<(String, String)> = potential_duplicates
        .par_iter()
        .flat_map(|group| {
            if cancelled.load(Ordering::Relaxed) {
                return vec![];
    ...
    potential_duplicates = partial_hash_map
        .into_values()
        .filter(|v| v.len() > 1)
        .collect();
    ...
    *processed_groups.lock().unwrap() = 0;
    ...
    let _ = window.emit(
        "scan-progress",
        ProgressEvent {
            phase: "Phase 2/2: Full Hash".into(),
            current: 0,
            total: total_groups_full,
        },
    );

    let full_results: Vec<(String, String)> = potential_duplicates
        .par_iter()
        .flat_map(|group| {
            if cancelled.load(Ordering::Relaxed) {
                return vec![];
